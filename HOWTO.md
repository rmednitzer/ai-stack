...
openwebui:
  env:
    ...
    # (Open WebUI's vars are CHUNK_SIZE / CHUNK_OVERLAP — the RAG_-prefixed
    # names are the ingestion worker's separate contract, not Open WebUI's.)
    CHUNK_SIZE: "1500"
...
openwebui:
  env:
    ...
    # {id,type,title,content,dismissible,timestamp}. The older
    # WEBUI_BANNER_TEXT / WEBUI_BANNER_DISMISSIBLE vars are not read by current
    # Open WebUI (validated against open-webui config.py).
    WEBUI_BANNERS: '[{"id":"ai-act-disclosure","type":"info","title":"","content":"You are interacting with an AI-powered assistant. Responses are generated by a large language model and may not always be accurate.","dismissible":true,"timestamp":0}]'
```

Customise the `content` field for your deployment. Set `WEBUI_BANNERS: "[]"` to disable.
