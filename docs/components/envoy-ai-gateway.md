...
Governed, in-cluster OpenAI-compatible **model-egress gateway** (ADR-006). Turns a pre-existing Gateway into one endpoint that fronts local Ollama and external providers, centralizing provider credentials, routing, optional token-based rate limiting, and JWT/OIDC client auth. **Apache-2.0**, no open-core carve-out.
...
- **Default**: opt-in (`envoyAIGateway.enabled=false`)
- **Upstream**: <https://aigateway.envoyproxy.io/> · [docs](https://aigateway.envoyproxy.io/docs/)
...
- **Chart path**: [`templates/envoy-ai-gateway/`](../../templates/envoy-ai-gateway/)

## Runtime model — CR-only, BYO controller

This component is **configuration, not a workload**. The chart emits only the Envoy AI Gateway custom resources (`AIGatewayRoute`, `AIServiceBackend`, `BackendSecurityPolicy`, the Envoy Gateway `Backend`, and optional `BackendTrafficPolicy` / `SecurityPolicy`) and attaches them to a **pre-existing Gateway** — exactly as the opt-in `HTTPRoute` (ADR-003) attaches to a Gateway it does not provision.
...
| Key | Purpose |
|-----|---------|
| `envoyAIGateway.enabled` | Toggle the component (mutually exclusive with `externalAPIs`) |
| `envoyAIGateway.parentRefs` | The pre-existing Gateway(s) the `AIGatewayRoute` attaches to (required when enabled) |
| `envoyAIGateway.includeOllama` / `ollamaModel` | Route the in-cluster Ollama backend |
| `envoyAIGateway.providers[]` | External providers (`name`, `schema`, `hostname`, `model`, `apiKey`/`existingSecret`) |
| `envoyAIGateway.rateLimit.*` | Token-based rate limiting (`BackendTrafficPolicy` + `LLMRequestCosts`) |
| `envoyAIGateway.clientAuth.*` | JWT/OIDC client auth (`SecurityPolicy`; defaults to the Authelia issuer when `authelia.enabled`) |

## Secrets

Each provider's upstream API key is stored in a chart-managed `*-envoy-ai-gateway-<name>-secret` Secret (data key `apiKey`, `helm.sh/resource-policy: keep`) or referenced via `existingSecret` — never written into a rendered manifest.
