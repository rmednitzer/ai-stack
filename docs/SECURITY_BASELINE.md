...
| `readOnlyRootFilesystem: false` | Open WebUI, Ollama, Tika, SearXNG, Postgres, Authelia | Writable rootfs required at runtime; each drops all caps + `/tmp` `emptyDir` | `.kube-linter.yaml` `no-read-only-root-fs` exclusion |
